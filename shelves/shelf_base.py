...
class _shelf():
    ...
    def addButton(self, label, icon="commandButton.png", ann="", command=_null, doubleCommand=_null, overlayLabelColor=(.9, .9, .9)):
        ...
        cmds.shelfButton(width=37, height=37, ann=ann, image=icon, l=label, command=command, dcc=doubleCommand, imageOverlayLabel=label, olb=self.labelBackground, olc=self.labelColour)
    ...
    def addSeparator(self):
        cmds.separator(hr=False, st="none", w=4)
        cmds.separator(hr=False, st="none", w=2, ebg=True, bgc=[.5, .5, .5])
        cmds.separator(hr=False, st="none", w=4)
        return
